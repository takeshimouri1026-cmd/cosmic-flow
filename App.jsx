...
import { loadLogs, saveLog, deleteLog } from "./logStorage.js";
...
export default function App() {
  ...
  function analyze() {
    ...
    setData({
      week,
      awakening: awakeningScore(b),
      overall: overallEnergy(b),
    });
    setAdvice(null);
    setAnalysis(null);
  }
  ...
  async function getAdvice() {
    ...
    try {
      ...
      setError(e.message || "通信に失敗しました。サーバーが起動しているか確認してください。");
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleSaveLog() {
    ...
    const entry = { date: logDate, text: logText.trim() };
  ...
  async function getAnalysis() {
    ...
    try {
      const r = await fetch(`${API}/api/analyze`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...data, name, logs }),
      });
      const j = await r.json();
      if (j.error) throw new Error(j.error);
      setAnalysis(j);
  ...
  return (
    <div className="min-h-screen text-stone-100 px-5 py-10 md:px-10">
      <header className="max-w-3xl mx-auto text-center mb-10">
        <p className="tracking-[0.4em] text-amber-300/70 text-xs mb-3">
          COSMIC FLOW
        </p>
      ...
      {data && (
        <div className="max-w-3xl mx-auto mt-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Stat
              label="今週の総合エネルギー"
              value={data.overall > 0 ? "上昇の流れ" : "内省の流れ"}
              sub={`指数 ${Math.round(data.overall * 100)}`}
            />
            <Stat
              label="覚醒スコア"
              value={`${data.awakening} / 100`}
              sub={data.awakening > 60 ? "波長が整いやすい" : "静けさが鍵"}
            />
          </div>
          ...
          <div className="bg-white/5 rounded-2xl p-5 border border-amber-200/10">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={data.week}>
                ...
                <XAxis
                  dataKey="label"
                  stroke="#a8a29e"
                  tickFormatter={(v, i) => `${v}(${data.week[i].weekday})`}
                />
                ...
                />
                <Legend
                  formatter={(value) => <span style={{ color: "#d6d3d1", fontSize: 12 }}>{value}</span>}
                />
            ...
            <p className="text-xs text-stone-500 mt-2">
              点線(金色)が今日です。身体23日・感情28日・知性33日・直感38日の周期で計算しています。
            </p>
          </div>
          ...
          <div className="bg-white/5 rounded-2xl p-5 border border-amber-200/10 space-y-3">
            ...
            <div className="flex gap-3 items-center">
              <label className="text-xs text-stone-400 whitespace-nowrap">記録日</label>
              <input
                type="date"
            ...
            <div className="bg-white/5 rounded-2xl p-5 border border-amber-200/10 space-y-3">
              ...
              {showLogs && (
                <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                  {logs.map((l) => (
                    <div key={l.date} className="bg-black/20 rounded-xl p-4 border border-white/5">
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-xs text-amber-300/70">{l.date}</span>
                        <button
                          onClick={() => handleDeleteLog(l.date)}
                          className="text-xs text-stone-600 hover:text-rose-400 transition"
                        >
                          削除
                        </button>
                      </div>
