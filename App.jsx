...
export default function App({ session }) {
  ...
  const profileCard = (
    <div className="bg-white/[0.04] backdrop-blur-md rounded-2xl p-6 border border-white/10 shadow-[0_0_50px_rgba(120,110,200,0.08)]">
  ...
  return (
    <div className="min-h-screen text-stone-100 px-5 py-8 md:px-10">
      ...
      <header className={`max-w-3xl mx-auto text-center ${data ? "mb-6" : "mb-10"}`}>
      ...
      {data && (
        <div className="max-w-3xl mx-auto">
          {/* タブバー（上部に固定） */}
          <nav className="sticky top-3 z-20 mb-6">
            <div className="flex gap-1 bg-white/[0.06] backdrop-blur-xl rounded-full p-1 border border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.35)]">
              {TABS.map((t) => (
                <button
                  key={t.id}
                  onClick={() => setTab(t.id)}
                  className={`flex-1 rounded-full py-2 text-sm transition ${
                    tab === t.id
                      ? "bg-amber-300 text-stone-900 font-medium"
